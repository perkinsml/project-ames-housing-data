...
```
    ...
    <matplotlib.figure.Figure at 0x114b9cdd8>
...
##### Data Cleaning: BsmtExposure field
...
![png](output_118_12.png)



![png](output_118_13.png)
...
I'll now investigate and resolve the null values in these features.
...
Before modeling the residuals I'll examine the relationships between the changeable features and the residuals.  This time, I'll simulate a hold-out test set by only looking at the relationships within the training data.

I'll first create the train and test sets with both the chanegable features and the residuals.
...
Both OverallQual and OverallCond have relatively low positive correlations with the Residuals.  Interestingly, the correlation with the Sale Price is higher, meaning that there is a stronger relationship between the Sale Price and the Residual.
...
![png](output_255_13.png)



![png](output_255_14.png)



![png](output_255_15.png)



![png](output_255_16.png)



![png](output_255_17.png)



![png](output_255_18.png)



![png](output_255_19.png)



![png](output_255_20.png)



![png](output_255_21.png)
...
```
    ...
    {0: 'OverallQual', 1: 'OverallCond', 2: 'RoofStyle', 3: 'RoofMatl', 4: 'Exterior1st', 5: 'Exterior2nd', 6: 'MasVnrType', 7: 'ExterQual', 8: 'ExterCond', 9: 'BsmtCond', 10: 'BsmtFinType1', 11: 'BsmtFinType2', 12: 'Heating', 13: 'HeatingQC', 14: 'CentralAir', 15: 'Electrical', 16: 'KitchenQual', 17: 'Functional', 18: 'FireplaceQu', 19: 'GarageFinish', 20: 'GarageQual', 21: 'GarageCond', 22: 'PoolQC', 23: 'Fence', 24: 'YrSold', 25: 'SalePrice', 26: 'Residuals'}
...
Before dummifying the categorical features, I'll need to stack the train and test data as the same feature values may not exist in the train and test data.  Dumifying the data separately may result in dataframes of different dimensions.
...
X_house_reno_train= X_house_reno.iloc[:1220,:]
X_house_reno_test= X_house_reno.iloc[1220:,:]
...
```
    ...
    <class 'pandas.core.frame.DataFrame'> <class 'pandas.core.frame.DataFrame'> <class 'pandas.core.series.Series'> <class 'pandas.core.series.Series'>
    (1220, 66) (164, 66) (1220,) (164,)


Data types and shapes are aligned with expectations.
...
I will now fit a model to the training data and evaluate it on the test data.
...
lnr_r.fit(Xr_train_ss, y_residuals_train)
print(f"R2 score on training data: {round(lnr_r.score(Xr_train_ss, y_residuals_train),3)}")
print(f"R2 score on test data: {round(lnr_r.score(Xr_test_ss, y_residuals_test),3)}")
```

    R2 score on training data: 0.222
    R2 score on test data: -2.1679476163090906e+18


Model reports a relatively low R2 score on the training data, and large negative R2 score on the test data, 
...
lasso_r = LassoCV(alphas=np.logspace(-4,3,5000), cv=KFold(n_splits=5, shuffle=True, random_state=5), 
...
The R2 score on the training data is slightly lower than the standard Linear Regression, but this is to be expected since the cross validation means that we're not fitting the data to all of the training data.  Hopefully this results in a model that generalises better to the training data.
...
```
    ...
    R2 score of optimum Lasso on test data: 0.151
...
```python
X_reno_dumified.columns
```




    Index(['OverallQual', 'OverallCond', 'YrSold', 'SalePrice', 'RoofStyle_Gable',
           'RoofStyle_Gambrel', 'RoofStyle_Hip', 'RoofStyle_Mansard',
           'RoofStyle_Shed', 'RoofMatl_Membran',
           ...
           'GarageCond_NA', 'GarageCond_Po', 'GarageCond_TA', 'PoolQC_Fa',
           'PoolQC_Gd', 'PoolQC_NA', 'Fence_GdWo', 'Fence_MnPrv', 'Fence_MnWw',
           'Fence_NA'],
...
```
    ...
    Percent of predictor coefficients zeroed out by Lasso regularisation: 45.45%.


45% of predictor coefficients were zeroed out by the Lasso regularisation confirming the speculation 
above that the model was overfitting the training data.
...
The chart above provides further evidence as to why the model's accuracy in predicting the residuals is relatively poor.  2 of the 3 most important/significant 
predictors are OverallQual and OverallCond which both had a relatively low correlation with the target (residuals) itself!

We are predicting the dollar amount now, but note it is in terms of ERRORS of the first model.
If the first model overestimated the price for a house, then the error in price (residual) would be negative. 
We are predicting these residuals. The -\$6740 coefficient for KitchenQual_Gd means this feature is "worth"
-\$6740 dollars i.e it is predicted to be -\$6740 dollars greater by the model.
...
model's ability to accurately identify which properties to renovate and how.  Fixed features can be modeled with significantly more accuracy than changeable features.
...
```python
# Display value counts for SaleCondition field
...
```python
# Print baseline accuracy of predicting the majority class (ie. Normal)
prob_majority_class = house_df['SaleCondition'].value_counts().max() / house_df['SaleCondition'].count()
...
```python
# Print baseline accuracy of predicting the absnormal sale 
prob_abnormal_class = ((house_df['SaleCondition'] == 'Abnorml').sum()) / house_df['SaleCondition'].count()
...
X = house_df[['SalePrice', 'Neighborhood']]
y = house_df['SaleCondition']
```
...
```
    ...
    array([0.82598039, 0.83292383, 0.83703704])
...
```
    ...
    array([0.75735294, 0.77149877, 0.78765432])
...
Score is suprisingly(!) good, especially given simplicity of model however the baseline accuracy was 0.83, so the model isn't really doing much better than simply predicting the majority class.


```python
With more time, I'd bootstrap, while downsampling. 
```
